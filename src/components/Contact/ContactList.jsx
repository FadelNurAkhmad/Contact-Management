...
export default function ContactList() {
  ...
  // useEffectOnce Ini hanya dijalankan satu kali saat komponen mount (mirip componentDidMount).
  // Mounting = ketika komponen pertama kali ditampilkan atau dirender ke layar (DOM).
  useEffectOnce(() => {
    ...
    // Untuk cleanup saat komponen dihapus.
    return () => {
    ...
    <>
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          {/* Contact Card 1 */}
          {/* map() : Mengiterasi array, Mengembalikan array baru berisi hasil
          dari setiap elemen yang diproses. */}
        ...
        <div className="mt-10 flex justify-center">
          <nav className="flex items-center space-x-3 bg-gray-800 bg-opacity-80 rounded-xl shadow-custom border border-gray-700 p-3 animate-fade-in">
            ...
            {/*  Setiap elemen diberi key={} untuk performa React (wajib saat mapping list). */}
            {getPages().map((value) => {
              if (value === page) {
                return (
                  <a
